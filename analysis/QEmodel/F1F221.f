...
c      if(f2dqe.LT.0.0.OR.f1dqe.LT.0.0.OR.fLdqe.LT.0.0)
c     &    write(34,*) w2,q2,f2dqe,f1dqe,fLdqe
...
c      if(ymin.EQ.0) write(6,*) "Error ymin = 0" 
...
*
      ...
      if (n.gt.nmax) then
c         print*, 'ERROR(polintnum): order larger than max', n,'>', nmax 
         stop
      end if
      ns=1
      dif=abs(x-xa(1))
