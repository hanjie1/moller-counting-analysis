...
// Define Files
//////////////////////////////////////////////////////////////////////////////
//TString RootDir = "/lustre/expphy/work/halla/gmp12/yang/optics/optics_rootfiles/left/Loop111516/";
TString RootDir = "/lustre/expphy/work/halla/gmp12/yang/optics/optics_rootfiles/left/Loop122016/";

// Dp & Sieve
TString RootFile_Dp_m4 = "left_gmp_13834.root left_gmp_13834_1.root";
TString RootFile_Dp_m3;
TString RootFile_Dp_m2 = "left_gmp_13835.root left_gmp_13835_1.root";
TString RootFile_Dp_m1;
TString RootFile_Dp_0 = "left_gmp_13836.root left_gmp_13836_1.root";
TString RootFile_Dp_p1;
TString RootFile_Dp_p2 = "left_gmp_13837.root left_gmp_13837_1.root";
TString RootFile_Dp_p3;
TString RootFile_Dp_p4 = "left_gmp_13838.root left_gmp_13838_1.root";

TString RootFile_Sieve = "left_gmp_13831.root left_gmp_13831_1.root left_gmp_13831_2.root left_gmp_13831_3.root left_gmp_13832.root left_gmp_13832_1.root left_gmp_13832_2.root left_gmp_13832_3.root";
...
//TCut GeneralSieveCut = "abs(L.tr.tg_th)<0.15 && abs(L.tr.tg_ph)<0.15&& L.vdc.u1.nclust==1&& L.vdc.v1.nclust==1&& L.vdc.u2.nclust==1&& L.vdc.v2.nclust==1"; // && urb.y<0.006

TCut GeneralCut = "L.tr.n==1&&(L.prl2.e+L.prl1.e)/(1000*L.tr.p)>0.6&&(L.cer.asum_c>600) "; 
...
void CutVertex(int overwrite = 0) {
    ...
    //    TH2F* h3 = new TH2F("h3", "Tg_ph vs. L.tr.vz",  400, vzlowlimit, vzuplimit, 400, phlowlimit, phuplimit);
    assert(h3);
	...
	T->Draw("L.tr.vz:L.tr.tg_ph>>h3", DrawCut, "COLZ");
	//	T->Draw("L.tr.tg_ph:L.tr.vz>>h3", DrawCut, "COLZ");
	c3->Update();
        
        cout << "Testing " << Form("fcut_L_%d", FoilID) << endl;
        TCutG* cutg = (TCutG*)gROOT->FindObject(Form("fcut_L_%d", FoilID)); //looking for old cut definition
        
        if(!cutg || overwrite){
            cout << "Making cut " << Form("fcut_L_%d", FoilID) << endl;

            cutg = (TCutG*)(TVirtualPad::Pad()->WaitPrimitive("CUTG", "CutG")); // making cut, store to CUTG
            c3->Update();
	    
            cutg->SetName(Form("fcut_L_%d", FoilID)); //
	    cutg->SetVarX("L.tr.tg_ph");
	    cutg->SetVarY("L.tr.vz");
	    //	    cutg->SetVarY("L.tr.tg_ph");
	    //           cutg->SetVarX("L.tr.vz");

            cout << "done!" << endl;
        }
        else{
            cout << Form("fcut_L_%d", FoilID) << " is found, using old one" << endl;
	    }
        ...
        cutdesc << Form("fcut_L_%d", FoilID) << " && " << (const char*)GeneralCut << endl;

        SaveCanvas(c3, WorkDir + RootFileName + Form(".fcut_L_%d", FoilID), kFALSE);
    }
...
void CutVertex_Dp(int overwrite = 0) {
	...
	T->Draw("L.tr.vz:L.tr.tg_ph>>h3", DrawCut, "COLZ");
	//	T->Draw("L.tr.vz>>h3", DrawCut, "COLZ");
	c3->Update();
...
void CutDp(int overwrite = 0)
{
    ...
    for(int FoilID = 0; FoilID < nFoil; FoilID++){
        TCut DrawCut = GeneralCut;
        if(UseVertexCut){
            DrawCut = TCut(Form("fcut_L_%d", FoilID));
            (TCutG*)gROOT->FindObject(Form("fcut_L_%d", FoilID));
        }
        
	T->Draw("L.tr.tg_th:L.tr.tg_dp>>h4", DrawCut, "COLZ");
        c4->Update();

        cout << "Testing " << Form("dpcut_L_%d", FoilID) << endl;
        TCutG *cutg = (TCutG*)gROOT->FindObject(Form("dpcut_L_%d", FoilID));

        if(!cutg || overwrite){
            cutg = (TCutG*)(TVirtualPad::Pad()->WaitPrimitive("CUTG", "CutG")); // making cut, store to CUTG
            c4->Update();

            cutg->SetName(Form("dpcut_L_%d", FoilID));
            // set axises' name
            cutg->SetVarX("L.tr.tg_dp");
            cutg->SetVarY("L.tr.tg_th");
            cout << "done!" << endl;
        }
        else {
            cout << Form("dpcut_L_%d", FoilID) << " is found, using old one" << endl;
        }
        ...
        if(UseVertexCut){
            cutdesc << Form("fcut_L_%d", FoilID) << " && ";
        }
        cutdesc << Form("dpcut_L_%d", FoilID) << " && " << (const char*)GeneralCut << endl;

        SaveCanvas(c4, WorkDir + RootFileName + Form(".dpcut_L_%d", FoilID), kFALSE);
    }
...
void CutDp_Dp(int overwrite = 0)
{
	...
	T->Draw("L.tr.tg_ph:L.tr.tg_dp>>h4", DrawCut, "COLZ");
        c4->Update();
...
void CutSieve(int FoilID = 0, int col = 6, int overwrite = 0) {
    ...
    TString PlotDir(RootFileName + Form(".hcut_L_%d_%d/", FoilID, col));
    ...
    TCut DrawCut = GeneralCut;
    if(UseVertexCut){
        DrawCut = DrawCut + TCut(Form("fcut_L_%d", FoilID));
        (TCutG*)gROOT->FindObject(Form("fcut_L_%d", FoilID));
    }
    if(UseDpCut){
        DrawCut = DrawCut + TCut(Form("dpcut_L_%d", FoilID));
        (TCutG*)gROOT->FindObject(Form("dpcut_L_%d", FoilID));
    }

    T->Draw("L.tr.tg_th:L.tr.tg_ph>>h5", DrawCut, "COLZ");
    c5->Update();
    ...
    for(int row = rmin; row < rmin + nhol; row++){
        cout << "Testing " << Form("hcut_L_%d_%d_%d", FoilID, col, row) << endl;
	TCutG* cutg = (TCutG*)gROOT->FindObject(Form("hcut_L_%d_%d_%d", FoilID, col, row));
	    ...
	    cutg->SetPoint(4,ph_peak[col]-ph_width+0.001*(row-3)/2,th_peak[row]-th_width);
	      ...
	      */
            ...
            cutg->SetName(Form("hcut_L_%d_%d_%d", FoilID, col, row));
            // set axises' name
            cutg->SetVarX("L.tr.tg_ph");
            cutg->SetVarY("L.tr.tg_th");
            cout << "done!" << endl;
            ...
            cout << Form("hcut_L_%d_%d_%d", FoilID, col, row) << " is found, using old one" << endl;
        }
        ...
        if(UseHDpCut){
            ...
            TCut DpDrawCut = DrawCut + Form("hcut_L_%d_%d_%d", FoilID, col, row);
            (TCutG*)gROOT->FindObject(Form("hcut_L_%d_%d_%d", FoilID, col, row));
            T->Draw("L.tr.tg_dp>>h6", DpDrawCut);
            c6->Update();
            ...
            if(npeak==0){
                ...
                SaveCanvas(c6, WorkDir + PlotDir + RootFileName + Form(".hdpcut_L_%d_%d_%d", FoilID, col, row), kFALSE);
                c5->cd();
                ...
                cout << lowthr << "<L.tr.tg_dp<" << hithr << endl;
                ...
                cutdesc << Form("(L.tr.tg_dp>%7.5f && L.tr.tg_dp<%7.5f) && ", lowthr, hithr);
            }
        }
        
        cout << "Log to " << CutDescName << endl;

        cutdesc << Form("hcut_L_%d_%d_%d", FoilID, col, row) << " && ";
        if(UseVertexCut){
            cutdesc << Form("fcut_L_%d", FoilID) << " && ";
        }
        if(UseDpCut)
            cutdesc << Form("dpcut_L_%d", FoilID) << " && ";
    ...
    gSystem->Exec("rm -f " + WorkDir + PlotDir + RootFileName + Form(".hcut_L_%d_%d_*", FoilID, col) + ".*");
    SaveCanvas(c5, WorkDir + PlotDir + RootFileName + Form(".hcut_L_%d_%d_%d", FoilID, col, nhol), kFALSE);
