...
THaTrackingDetector(name, description, apparatus)
{
    ...
    //fPointingOffset, HCS
    fPointingOffset.SetXYZ(MissPointZ*TMath::Cos(HRSAngle), MissPointY, -MissPointZ * TMath::Sin(HRSAngle)); 
...
Int_t LOpticsOpt::LoadDataBase(TString DataBaseName)
{
    ...
    // Look for the section [<prefix>.global] in the file, e.g. [ L.global ]
    TString tag(fPrefix);
...
TCanvas * LOpticsOpt::CheckSieveAccu(Int_t PlotFoilID)
{
  ...
  //fit one-dimension projection
   ...
   //fit one-dimension projection
    ...
    fprintf(fp2," The uncorrected hozitontal postion of each column.\n");
    for (UInt_t idy = 0; idy < NSieveCol; idy++) {
      fprintf(fp2,"%*.2f", 7,1000*( GetSieveHoleTCS(NSieveCol-1-idy, 0).Y() ) );
    }
    fprintf(fp2,"\n");
    fprintf(fp2,"\n\n");

    fprintf(fp2," The uncorrected vertical postion of each row.\n");
    for (UInt_t idx = 0; idx < NSieveRow; idx++) {
      fprintf(fp2,"%*.2f", 6,1000*( GetSieveHoleTCS(0, NSieveRow-1-idx).X() ) );
    }
    fprintf(fp2,"\n");
    fprintf(fp2,"\n\n");
...
TCanvas * LOpticsOpt::CheckVertex()
{
	...
	for(UInt_t ncol =0; ncol<NSieveCol; ncol++){
	  for(UInt_t nrow=0; nrow<NSieveRow; nrow++){
	    HTgYHole[idx][ncol][nrow] = new TH1D(Form("TgY%d_%d_%d", idx, ncol, nrow), Form("TgY%d_%d_%d", idx, ncol, nrow), 1000, -YRange, YRange);
	    HTgYHoleReal[idx][ncol][nrow] = new TH1D(Form("TgY%d_%d_%d", idx, ncol, nrow),Form("TgY%d_%d_%d", idx, ncol, nrow) , 1000, -YRange, YRange);
    ...
    Double_t DefResolution = 1e-3;
    Double_t FitRangeMultiply = 2;
    Double_t ymean =0;
    for (UInt_t idx = 0; idx < nplot; idx++) {
        // UInt_t FoilID = idx;
	...
	TPaveText *t1 = new TPaveText(0.6,0.7,0.88,0.82,"NDC");
    ...
    for (UInt_t idx = 0; idx < NFoils; idx++) {
        HTgZ[idx] = new TH1D(Form("Target_Z%d", idx), Form("Target Z for Foil Target #%d", idx), 400, -ZRange, ZRange);
        HTgZReal[idx] = new TH1D(Form("Target_Z%d", idx), Form("Target Z for Foil Target #%d", idx), 400, -ZRange, ZRange);
	...
	CalcReacZ = - ( eventdata.Data[kCalcTgY] + MissPointZ)*TMath::Cos(TMath::ATan(Real_Tg_Phi))/TMath::Sin(HRSAngle + TMath::ATan(Real_Tg_Phi)) + BeamSpotHCS.X()*TMath::Cos(HRSAngle+TMath::ATan(Real_Tg_Phi))/TMath::Sin(HRSAngle+TMath::ATan(Real_Tg_Phi));
	...
	TPaveText *tt = new TPaveText(0.6,0.7,0.88,0.82,"NDC");
...
void LOpticsOpt::PrepareDp(void)
{
	...
	//	cout<<"eventdata.Data[kCutID]:"<<eventdata.Data[kCutID]<<endl;
	...
	Double_t x_fp = eventdata.Data[kX];// L.tr.r_x
        const Double_t(*powers)[5] = eventdata.powers;
	...
	//	cout<<"eventdata.Data[kElossDp]:"<<eventdata.Data[kElossDp]<<endl;
	...
	//	cout<<"eventdata.Data[kRealDpKin]:"<<eventdata.Data[kRealDpKin]<<endl;
	//******************//
...
TCanvas * LOpticsOpt::CheckDp()
{
    ...
    for (UInt_t KineID = 0; KineID < NKine; KineID++) {
      ...
      hRealReactZ[KineID] = new TH1D(Form("hRealReactZ%d", KineID), Form("ReacZ for Delta Scan Kine. %d%%", 2*(KineID-2)), 400, -0.15, 0.2);
      ...
      hElossTg_ReactZ_After[KineID] = new TH2D(Form("hElossTg_ReacZ%d", KineID), Form("ElossTg for Delta Scan Kine. %d%%", 2*(KineID-2)), 400,-0.15,0.15,400, 0, 7);
      ...
      hTravelLength_ReactZ_After[KineID] = new TH2D(Form("hElossTg_ReacZ%d", KineID), Form("TravelLength vs ReacZ After for Delta Scan Kine. %d%%", 2*(KineID-2)), 400,-0.15,0.15,800, 0, 0.15);
	...
	hDpKinCalib[KineID]->GetXaxis()->SetRangeUser(AverCalcDpKin[KineID]-0.02,  AverCalcDpKin[KineID] +0.02 );
